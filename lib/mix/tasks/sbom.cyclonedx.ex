defmodule Mix.Tasks.Sbom.Cyclonedx do
  ...
  @default_schema "1.6"
  @default_top_component_type "application"

  @default_opts [
    output: @default_path,
    schema: @default_schema,
    type: @default_top_component_type
  ]
  ...
  ## Options
    ...
    * `--schema` (`-s`): schema version to be used, defaults to
      "#{@default_schema}"
    * `--type` (`-t`): the project type, e.g. "application", "library",
      "framework", defaults to "#{@default_top_component_type}"
  ...
  def run(all_args) do
    {opts, _args} =
      OptionParser.parse!(
        all_args,
        aliases: [o: :output, f: :force, d: :dev, r: :recurse, s: :schema, t: :type],
        strict: [
          output: :string,
          force: :boolean,
          dev: :boolean,
  ...
  defp generate_bom(output_path, environment, opts) do
    ...
    case SBoM.components_for_project(type, environment) do
      {:ok, components} ->
